					...
					if v2['HowMuch'] == v2['have'] and v2['complete'] == 0 then
						if Quests.npcArray[key]['particle'][nPlayerID] == false and v2['complete'] == 0 then
							...
							if v1['tasks'][tostring(k2+1)] == nil then
								Quests:addParticle(Quests.complite_quest_main, v2["UnitName"], key, nPlayerID)
							else
								Quests:addParticle(Quests.has_quest_main, v2["UnitName"], key, nPlayerID)
							end
						end
					...
					if v2['HowMuch'] == v2['have'] and v2['complete'] == 0 then
						if Quests.npcArray[key]['particle'][nPlayerID] == false and v2['complete'] == 0 then
							...
							if v1['tasks'][tostring(k2+1)] == nil then
								...
								Quests:addParticle(Quests.has_quest_bonus, v2["UnitName"], key, nPlayerID)
							end
						end
					elseif v2['HowMuch'] ~= v2['have'] and v2['active'] == 1 and Quests.npcArray[key]['particle'][nPlayerID] ~= false then
						Quests:deliteParticle(key, nPlayerID)
...
function Quests:deliteParticle(key, nPlayerID)
...
function Quests:addParticle(url, name, key, nPlayerID)
...
function Quests:acceptButton(t)
	...
	elseif player_info[tostring(steamID)][tostring(t.type)][tostring(t.number)]['tasks'][tostring(t.task)] == nil then
		...
		if t.type == 'bonus' and t.number == 9 then
			--print("giveReward//giveReward//giveReward//giveReward//giveReward//giveReward//")
			local selectedItem = player_info[tostring(steamID)][tostring(t.type)][tostring(t.number)]["selectedItem"]
			--print(selectedItem)
			player_info[tostring(steamID)][tostring(t.type)][tostring(11)]["selectedItem"] = selectedItem
			...
			player_info[tostring(steamID)][tostring(t.type)][tostring(19)]["selectedItem"] = selectedItem
		end
