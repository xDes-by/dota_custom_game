...
function Forge:init()
    ...
    CustomGameEventManager:RegisterListener("ForgeUpdateAvailability",function(_, keys)
        self:UpdateAvailability(keys)
    end)
end
...
function Forge:GatherItemDataArray(pid, item)
    ...
    local data = {
        ...
        availability = self:IsEnoughMoney(pid, item) and self:EnoughSouls(pid, item)
    }
    return data
end
    ...
    for ITEM_SLOT = DOTA_ITEM_SLOT_1, DOTA_STASH_SLOT_6 do
        ...
        if item then
            if table.has_value(self.upgradeableItems, item:GetName()) then
                table.insert(NewItemsList, self:GatherItemDataArray(PlayerID, item))
            end
...
function Forge:UpdateItemData(PlayerID, entindex)
    ...
    for key, data in pairs(self.PlayerItems[PlayerID]) do
        if data.entindex == entindex then
            self.PlayerItems[PlayerID][key] = self:GatherItemDataArray(PlayerID, item)
        end
    end
end
...
function Forge:UpdgradeButton(t)
    ...
    if self:EnoughSouls(t.PlayerID, item) and self:IsEnoughMoney(t.PlayerID, item) then
...
function Forge:IsEnoughMoney(pid, item)
    if item:GetLevel() >= self.levelMax then return false end
    local hero = PlayerResource:GetSelectedHeroEntity( pid )
    if hero:GetTotalGold() >= self:GetItemUpgradeCost(item:GetLevel()) then
        return true
    end
    return false
end
function Forge:EnoughSouls(pid, item)
    if self:GetItemUpgradeSoulsCost() <= 0 then return true end
    local hero = PlayerResource:GetSelectedHeroEntity( pid )
    local soul_name = self:GetSoulNameForItemUpgrade(item:GetName(), item:GetLevel())
    if hero:FindItemInInventory(soul_name) or sInv:HasSoul(soul_name, pid) then return true end
    return false
end
function Forge:UpdateAvailability(t)
    local pid = t.PlayerID
    self:ExploreAllInventory(pid)
    CustomGameEventManager:Send_ServerToPlayer( PlayerResource:GetPlayer( pid ), "UpdateAvailabilityCallback", {} )
end
Forge:init()