...
function Pets:init()
    ...
    for i = 0, 4 do
        self.change_availability[i] = false
...
function Pets:EquipPanoramaEvent(t)
    ...
    local name = t.name
    if self.player[pid][name].value <= 0 then return end
    if not (self.change_limit[pid] > 0 or self.change_availability[pid] == true) then return end
    local equip = self:Equip(pid, name)
    if equip and self.change_availability[pid] == false then
        self.change_limit[pid] = self.change_limit[pid] -1
    end
    CustomNetTables:SetTableValue( "cosmetic_buttons", "pet", self.current_pet_spell )
    -- CustomNetTables:SetTableValue("player_pets", tostring(pid), {pet = self.current_pet_spell[pid]})
end
...
function Pets:OnBuyPet(t)
    ...
    elseif pet.price.don ~= nil and pet.price.don > 0 then
        Shop.pShop[pid]["mmrpoints"] = Shop.pShop[pid]["mmrpoints"] - pet.price.don
    ...
    DataBase:Send(DataBase.link.BuyPet, "GET", send, pid, not DataBase:IsCheatMode(), function(body)
        ...
        CustomNetTables:SetTableValue('Pets', tostring(pid), self.player[pid])
        DeepPrintTable(self.player[pid])
    end)
end
