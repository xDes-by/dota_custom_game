...
function talants:selectTalantButton(t)
    ...
    if GameRules:State_Get() >= DOTA_GAMERULES_STATE_PRE_GAME then
        ...
        if (t.j == 8 or t.j == 7 or t.j == 6) and ( tonumber(tab[t.i .. 6]) == 1 or tonumber(tab[t.i .. 7]) == 1 or tonumber(tab[t.i .. 8]) == 1 or tonumber(tab[t.i .. 5]) ~= 1 ) then talants:FastLearning(t) return end
...
function talants:FastLearning(t)
    ...
    if (t.i == 'don' and tab['freedonpoints'] >= levelNeed - branch_count) or (t.i ~= 'don' and tab['freepoints'] >= levelNeed - branch_count) then
        ...
        if (t.j >= 6 or t.j >= 7 or t.j >= 8) and tab[t.i..1] ~= 5 then
