...
function ability_npc_boss_plague_squirrel_spell9:OnSpellStart()
    ...
    local units = FindUnitsInRadius(self:GetCaster():GetTeamNumber(), pos, nil, radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)
    ...
    for _,unit in pairs(units) do
        ...
        self:GetCaster():AddNewModifier(self:GetCaster(),self,"modifier_generic_arc_lua",
		    {target_x = point.x,target_y = point.y,distance = 300,duration = 0.4,height = 350,fix_end = true,activity = ACT_DOTA_FLAIL})
