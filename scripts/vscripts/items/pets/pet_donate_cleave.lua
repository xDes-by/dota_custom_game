LinkLuaModifier( "modifier_donate_pet_cleave", "items/pets/pet_donate_cleave", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_item_pet_donate_cleave", "items/pets/pet_donate_cleave", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier("modifier_item_bfury_lua_debuff", 'items/custom_items/item_bfury_lua.lua', LUA_MODIFIER_MOTION_NONE)

spell_item_pet_donate_cleave = class({})

function spell_item_pet_donate_cleave:OnSpellStart()
	if IsServer() then
		...
		self.caster,
		self,
		"modifier_donate_pet_cleave", 
		{})
		EmitSoundOn( "Hero_Lion.Voodoo", self:GetCaster() )
	end
end

function spell_item_pet_donate_cleave:GetIntrinsicModifierName()
	return "modifier_item_pet_donate_cleave"
end

modifier_item_pet_donate_cleave = class({})

function modifier_item_pet_donate_cleave:IsHidden()
	return true
end

function modifier_item_pet_donate_cleave:IsPurgable()
	return false
end

function modifier_item_pet_donate_cleave:OnCreated( kv )
		if IsServer() then
	local point = self:GetCaster():GetAbsOrigin()
	if not self:GetCaster():IsIllusion() then
	self.pet = CreateUnitByName("pet_donate_cleave", point, true, nil, nil, DOTA_TEAM_GOODGUYS)
	self.pet:SetControllableByPlayer(self:GetCaster():GetPlayerID(), true)
	self.pet:SetOwner(self:GetCaster())
	end
	end
end
function modifier_item_pet_donate_cleave:OnDestroy()
	UTIL_Remove(self.pet)
end
function modifier_item_pet_donate_cleave:DeclareFunctions()
	local funcs =
	{
...
function modifier_item_pet_donate_cleave:OnAttackLanded(keys)
    if not (
        IsServer()
...
function modifier_item_pet_donate_cleave:PlayEffects1(direction )
...
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------------------------------------------------------------------
modifier_donate_pet_cleave = class({})

function modifier_donate_pet_cleave:IsHidden()
	return true
end

function modifier_donate_pet_cleave:IsPurgable()
	return false
end

function modifier_donate_pet_cleave:OnCreated( kv )
	self.caster = self:GetCaster()
...
function modifier_donate_pet_cleave:OnRefresh( kv )
end

function modifier_donate_pet_cleave:OnRemoved(kv)
end

function modifier_donate_pet_cleave:OnDestroy()
end

function modifier_donate_pet_cleave:CheckState()
	local state = {
...
function modifier_donate_pet_cleave:DeclareFunctions()
	local funcs = {
...
function modifier_donate_pet_cleave:GetModifierModelChange(params)
 return "models/items/courier/faceless_rex/faceless_rex.vmdl"
end

function modifier_donate_pet_cleave:GetModifierMoveSpeed_Absolute()
	return self.speed
end

function modifier_donate_pet_cleave:GetModifierIncomingDamage_Percentage()
	return 300
end

function modifier_donate_pet_cleave:OnAttack( params )
	if IsServer() then
	if params.attacker~=self:GetParent() then return end
	--if params.target:GetTeamNumber()==self:GetParent():GetTeamNumber() then return end

	local modifier = self:GetParent():FindModifierByNameAndCaster( "modifier_donate_pet_cleave", self:GetParent() )
...
function modifier_donate_pet_cleave:OnSpentMana( params )
	if IsServer() then
	local ability = self:GetAbility()
