...
LinkLuaModifier( "modifier_meat_hook_followthrough_lua", "heroes/hero_pudge/hook/hook_spell.lua" ,LUA_MODIFIER_MOTION_NONE )
...
function pudge_meat_hook_lua:OnSpellStart()
	...
	if self:GetCaster():FindAbilityByName("npc_dota_hero_pudge_agi10") ~= nil then
		self.hook_damage = self:GetSpecialValueFor( "hook_damage" ) + self:GetCaster():GetBaseDamageMin()
	end
