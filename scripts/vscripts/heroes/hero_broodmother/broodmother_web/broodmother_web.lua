...
function damage_tlents(ability, caster) 
	...
	if caster:FindAbilityByName("npc_dota_hero_broodmother_agi6") then
		damage = ability:GetSpecialValueFor("damage") + caster:GetAgility()
	end
	
	if caster:FindAbilityByName("npc_dota_hero_broodmother_int7") then
		damage = ability:GetSpecialValueFor("damage") + caster:GetIntellect()
	end
	return damage
end
...
function modifier_broodmother_web_talent:IsDebuff() return true end
function modifier_broodmother_web_talent:IsHidden() return false end
...
function modifier_broodmother_web:OnCreated()
	if IsServer() and self:GetCaster():FindAbilityByName("npc_dota_hero_broodmother_int10") then
...
function modifier_broodmother_web:OnIntervalThink()
	...
	self.damageTable = {
		...
		damage = damage_tlents(self:GetAbility(), self:GetCaster()) ,
