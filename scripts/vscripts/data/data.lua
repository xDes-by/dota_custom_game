forest_mini = {"forest_creep_mini_1","forest_creep_mini_2","forest_creep_mini_3"}
forest_big = {"forest_creep_big_1","forest_creep_big_2","forest_creep_big_3"}

village_mini = {"village_creep_1","village_creep_2"}
village_big = {"village_creep_3"}

mines_mini = {"mines_creep_1","mines_creep_2"}
mines_big = {"mines_creep_3"}

dust_mini = {"dust_creep_1","dust_creep_3","dust_creep_3"}
dust_big = {"dust_creep_2","dust_creep_4","dust_creep_6"}

cemetery_mini = {"cemetery_creep_1","cemetery_creep_3"}
cemetery_big = {"cemetery_creep_2","cemetery_creep_4"}

swamp_mini = {"swamp_creep_1","swamp_creep_3"}
swamp_big = {"swamp_creep_2","swamp_creep_4"}

snow_mini = {"snow_creep_1","snow_creep_2"}
snow_big = {"snow_creep_3","snow_creep_4"}

last_mini = {"last_creep_1","last_creep_2"}
last_big = {"last_creep_3","last_creep_4"}

scroll_item_lvl_1 = {"item_up_ability_tower"}
scroll_item_lvl_2 = {"item_up_ability_tower2"}
scroll_item_lvl_3 = {"item_up_ability_tower3"}
scroll_item_lvl_4 = {"item_up_ability_tower4"}

bosses_ability = {"boss_ursa_earthshock_lua","boss_ursa_overpower_lua","ursa_enrage_lua","boss_2_spray","boss_2_heart","boss_2_spawn","boss_2_poison","boss_3_frost_blast","boss_3_chain_frost","boss_3_shards","boss_3_slow","boss_4_hole",
"boss_4_eva","boss_4_midnignt","boss_4_eidolon","boss_5_lighting","boss_5_passive","boss_5_bolt","boss_5_passive2","boss_6_torrent","boss_6_armor","boss_6_ravage","boss_6_gush","boss_7_flux","boss_7_magnetic_field","boss_7_spark_wraith","boss_7_hide",
"boss_8_chrono","boss_8_time_delay","boss_8_illusion","boss_8_lock","boss_9_pssive","boss_9_orbs","boss_9_drain","boss_9_aura","boss_10_passive","boss_10_flame","boss_10_macropyre","boss_10_chain", "ability_npc_boss_barrack1_spell1", "ability_npc_boss_barrack1_spell2", "ability_npc_boss_barrack1_spell3", "ability_npc_boss_barrack1_spell4", "ability_npc_boss_barrack2_spell1", "ability_npc_boss_barrack2_spell2", "ability_npc_boss_barrack2_spell3", "ability_npc_boss_barrack2_spell4","npc_byorrocktar_spell1","npc_byorrocktar_spell2","npc_byorrocktar_spell3","npc_byorrocktar_spell4","npc_byorrocktar_spell5","zuus_boss_circle_lua","zuus_boss_nimbus"}

boss_belka = {"ability_npc_boss_plague_squirrel_spell1", "ability_npc_boss_plague_squirrel_spell2","ability_npc_boss_plague_squirrel_spell3","ability_npc_boss_plague_squirrel_spell4","ability_npc_boss_plague_squirrel_spell5", "ability_npc_boss_plague_squirrel_spell6", "ability_npc_boss_plague_squirrel_spell7"}

creep_ability = {"polar_furbolg_ursa_warrior_thunder_clap_lane_creep","ogreseal_flop_by","centaur_hoof_stomp_lane_creep","lycan_summon_wolves_critical_strike_lane_creep","skeleton_king_reincarnation_lane_creep","crystal_maiden_crystal_nova_lane_creep","venomancer_venomous_gale_lane_creep","bristleback_quill_spray_lane_creep","elder_titan_natural_order_lane_creep","mars_bulwark_lane_creep"}

------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

items_level_1 = {"item_crimson_guard_lua1","item_shivas_guard_lua1","item_desolator_lua1","item_skadi_lua1","item_mjollnir_lua1","item_veil_of_discord_lua1","item_monkey_king_bar_lua1","item_bfury_lua1","item_greater_crit_lua1","item_kaya_lua1","item_ethereal_blade_lua1","item_pipe_lua1"}
items_level_2 = {"item_crimson_guard_lua2","item_shivas_guard_lua2","item_desolator_lua2","item_skadi_lua2","item_mjollnir_lua2","item_veil_of_discord_lua2","item_monkey_king_bar_lua2","item_bfury_lua2","item_greater_crit_lua2","item_kaya_lua2","item_ethereal_blade_lua2","item_pipe_lua2"}
items_level_3 = {"item_crimson_guard_lua3","item_shivas_guard_lua3","item_desolator_lua3","item_skadi_lua3","item_mjollnir_lua3","item_veil_of_discord_lua3","item_monkey_king_bar_lua3","item_bfury_lua3","item_greater_crit_lua3","item_kaya_lua3","item_ethereal_blade_lua3","item_pipe_lua3"}
items_level_4 = {"item_crimson_guard_lua4","item_shivas_guard_lua4","item_desolator_lua4","item_skadi_lua4","item_mjollnir_lua4","item_veil_of_discord_lua4","item_monkey_king_bar_lua4","item_bfury_lua4","item_greater_crit_lua4","item_kaya_lua4","item_ethereal_blade_lua4","item_pipe_lua4"}
items_level_5 = {"item_crimson_guard_lua5","item_shivas_guard_lua5","item_desolator_lua5","item_skadi_lua5","item_mjollnir_lua5","item_veil_of_discord_lua5","item_monkey_king_bar_lua5","item_bfury_lua5","item_greater_crit_lua5","item_kaya_lua5","item_ethereal_blade_lua5","item_pipe_lua5"}
items_level_6 = {"item_crimson_guard_lua6","item_shivas_guard_lua6","item_desolator_lua6","item_skadi_lua6","item_mjollnir_lua6","item_veil_of_discord_lua6","item_monkey_king_bar_lua6","item_bfury_lua6","item_greater_crit_lua6","item_kaya_lua6","item_ethereal_blade_lua6","item_pipe_lua6"}

items_level_inv = {"item_crimson_guard_lua1","item_shivas_guard_lua1","item_desolator_lua1","item_skadi_lua1","item_mjollnir_lua1","item_veil_of_discord_lua1","item_monkey_king_bar_lua1","item_kaya_lua1","item_ethereal_blade_lua1","item_pipe_lua1"}
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------


Ability_tower_passive_lvl_1 = {"nevermore_dark_lord","vengefulspirit_command_aura","sniper_headshot","venomancer_poison_sting","abyssal_underlord_atrophy_aura","shredder_reactive_armor","meepo_ransack","meepo_geostrike","tidehunter_kraken_shell","faceless_void_backtrack"}

Ability_tower_passive_lvl_2 = {"viper_corrosive_skin","beastmaster_inner_beast","huskar_berserkers_blood","weaver_geminate_attack","warlock_golem_flaming_fists","abaddon_frostmourne",
"legion_commander_moment_of_courage","brewmaster_fire_permanent_immolation","chaos_knight_chaos_strike","tiny_grow","beastmaster_boar_poison","juggernaut_blade_dance","axe_counter_helix"}

Ability_tower_passive_lvl_3 = {"spectre_desolate","troll_warlord_fervor","lycan_feral_impulse","warlock_golem_permanent_immolation","ursa_fury_swipes","tiny_craggy_exterior","broodmother_incapacitating_bite","antimage_mana_break"}

Ability_tower_passive_lvl_4 = {"phantom_assassin_coup_de_grace","elder_titan_natural_order","sven_great_cleave","luna_moon_glaive","life_stealer_feast","templar_assassin_psi_blades","enchantress_untouchable","sandking_caustic_finale"}

AutoCastItem = {"item_black_king_bar","item_lotus_orb","item_ethereal_blade_lua1"}

abiility_passive = {"huskar_berserkers_blood","shredder_reactive_armor","antimage_mana_break","antimage_spell_shield","axe_counter_helix","earthshaker_aftershock", "juggernaut_blade_dance","lina_fiery_soul","nevermore_dark_lord","razor_unstable_current",
"sven_great_cleave","storm_spirit_overload","sandking_caustic_finale","tiny_craggy_exterior","tiny_grow","zuus_static_field","slardar_bash","tidehunter_kraken_shell","broodmother_incapacitating_bite", "lich_frost_aura","riki_backstab","sniper_headshot","necrolyte_heartstopper_aura","warlock_golem_flaming_fists","warlock_golem_permanent_immolation","broodmother_poison_sting", "beastmaster_boar_poison","beastmaster_inner_beast","venomancer_poison_sting","faceless_void_backtrack","faceless_void_time_lock","phantom_assassin_coup_de_grace","templar_assassin_psi_blades", "viper_corrosive_skin","dragon_knight_dragon_blood","life_stealer_feast","omniknight_degen_aura","enchantress_untouchable","huskar_berserkers_blood", "ursa_fury_swipes","lycan_feral_impulse","lycan_summon_wolves_critical_strike","brewmaster_fire_permanent_immolation","weaver_geminate_attack","mars_bulwark_lane_creep", "chaos_knight_chaos_strike","meepo_geostrike","meepo_ransack","treant_natures_guise","ogre_magi_multicast","rubick_arcane_supremacy","troll_warlord_fervor","centaur_return","shredder_reactive_armor","bristleback_bristleback", "abaddon_frostmourne","elder_titan_natural_order","legion_commander_moment_of_courage","abyssal_underlord_atrophy_aura","spectre_desolate"}


Ability_active_final = {"mars_arena_of_blood","custom_forest","tusk_snowball_meteor","custom_earth_splitter","earth_spirit_magnetize_datadriven","wisp_spirits_datadriven","custom_nyx_skill","sand_king_burrowstrike_lua","lich_sinister_gaze","custom_solar_flare","night_stalker_crippling_fear","creature_fire_breath"}

Ability_active = {
			"antimage_mana_void",
			"axe_berserkers_call",
			"axe_battle_hunger",
			"bane_brain_sap",
			"bane_fiends_grip",
			"bloodseeker_blood_bath",
			"earthshaker_fissure",
			"earthshaker_echo_slam",
			"juggernaut_blade_fury",
			"juggernaut_omni_slash",
			"kunkka_torrent",
			"kunkka_ghostship",
			"lina_dragon_slave",
			"lina_light_strike_array",
			"lina_laguna_blade",
			"lion_impale",
			"lion_voodoo",
			"lion_finger_of_death",
			"mirana_arrow",
			"mirana_starfall",
			"morphling_waveform",
			"puck_illusory_orb",
			"puck_waning_rift",
			"puck_dream_coil",
			"pudge_meat_hook",
			"pudge_dismember",
			"shadow_shaman_ether_shock",
			"shadow_shaman_voodoo",
			"shadow_shaman_shackles",
			"razor_plasma_field",
			"razor_eye_of_the_storm",
			"skeleton_king_hellfire_blast",
			"death_prophet_carrion_swarm",
			"death_prophet_silence",
			"death_prophet_exorcism",
			"sven_storm_bolt",
			"sven_gods_strength",
			"storm_spirit_static_remnant",
			"storm_spirit_electric_vortex",
			"sandking_burrowstrike",
			"sandking_sand_storm",
			"tiny_avalanche",
			"tiny_toss",
			"zuus_arc_lightning",
			"zuus_lightning_bolt",
			"zuus_thundergods_wrath",
			"slardar_sprint",
			"slardar_slithereen_crush",
			--"slardar_amplify_damage",
			"tidehunter_gush",
			"tidehunter_anchor_smash",
			"tidehunter_ravage",
			"vengefulspirit_magic_missile",
			"vengefulspirit_wave_of_terror",
			"vengefulspirit_nether_swap",
			"crystal_maiden_crystal_nova",
			"crystal_maiden_freezing_field",
			"windrunner_powershot",
			"windrunner_shackleshot",
			"lich_frost_nova",
			"lich_dark_sorcery",
			"lich_sinister_gaze",
			"lich_chain_frost",
			"witch_doctor_paralyzing_cask",
			"witch_doctor_maledict",
			"witch_doctor_death_ward",
			"riki_smoke_screen",
			"riki_blink_strike",
			"enigma_malefice",
			"tinker_laser",
			"tinker_heat_seeking_missile",
			"tinker_march_of_the_machines",
			"sniper_shrapnel",
			"sniper_assassinate",
			"necrolyte_death_pulse",
			"necrolyte_reapers_scythe",
			"warlock_fatal_bonds",
			"warlock_shadow_word",
			"beastmaster_primal_roar",
			"queenofpain_blink",
			"queenofpain_scream_of_pain",
			"queenofpain_sonic_wave",
			"venomancer_venomous_gale",
			"venomancer_poison_nova",
			"pugna_nether_blast",
			"phantom_assassin_stifling_dagger",
			"phantom_assassin_phantom_strike",
			"templar_assassin_refraction",
			"viper_nethertoxin",
			"viper_viper_strike",
			"luna_lucent_beam",
			"dragon_knight_breathe_fire",
			"dragon_knight_dragon_tail",
			"dazzle_poison_touch",
			"rattletrap_battery_assault",
			"leshrac_split_earth",
			"leshrac_diabolic_edict",
			"leshrac_lightning_storm",
			"leshrac_pulse_nova",
			"life_stealer_rage",
			"life_stealer_open_wounds",
			"dark_seer_vacuum",
			"broodmother_insatiable_hunger",
			"weaver_the_swarm",
			"jakiro_dual_breath",
			"jakiro_ice_path",
			"jakiro_macropyre",
			"jakiro_liquid_fire",
			"batrider_sticky_napalm",
			--"spectre_spectral_dagger",
			"doom_bringer_scorched_earth",
			"doom_bringer_doom",
			"ancient_apparition_cold_feet",
			"ancient_apparition_ice_vortex",
			"ancient_apparition_chilling_touch",
			"ursa_earthshock",
			"ursa_overpower",
			"ursa_enrage",
			"gyrocopter_rocket_barrage",
			"gyrocopter_flak_cannon",
			"gyrocopter_call_down",
			"alchemist_acid_spray",
			"alchemist_chemical_rage",
			"silencer_curse_of_the_silent",
			"silencer_global_silence",
			"brewmaster_thunder_clap",
			"brewmaster_drunken_haze",
			"brewmaster_cinder_brew",
			"brewmaster_drunken_brawler",
			"shadow_demon_soul_catcher",
			--"shadow_demon_demonic_purge",
			"chaos_knight_chaos_bolt",
			"chaos_knight_reality_rift",
			"treant_natures_grasp",
			"treant_leech_seed",
			"treant_overgrowth",
			"ogre_magi_fireblast",
			"ogre_magi_ignite",
			"rubick_telekinesis",
			"rubick_fade_bolt",
			--"disruptor_thunder_strike",
			"disruptor_kinetic_field",
			"disruptor_static_storm",
			"nyx_assassin_impale",
			"keeper_of_the_light_will_o_wisp",
			"slark_pounce",
			"medusa_mystic_snake",
			"centaur_hoof_stomp",
			"magnataur_shockwave",
			"magnataur_reverse_polarity",
			"bristleback_quill_spray",
			"tusk_ice_shards",
			"tusk_snowball",
			"tusk_walrus_punch",
			"skywrath_mage_arcane_bolt",
			"skywrath_mage_concussive_shot",
			"skywrath_mage_mystic_flare",
			"elder_titan_earth_splitter",
			"ember_spirit_searing_chains",
			--"abyssal_underlord_pit_of_malice",
			"pangolier_shield_crash",
			"dark_willow_bramble_maze",
			"dark_willow_cursed_crown",
			"grimstroke_soul_chain",
			"grimstroke_ink_creature",
			"grimstroke_dark_artistry",
			"mars_spear",
			"mars_gods_rebuke",
			"void_spirit_aether_remnant",
			"void_spirit_astral_step",
			"void_spirit_resonant_pulse",
			"snapfire_scatterblast",
			"snapfire_mortimer_kisses",
			"snapfire_firesnap_cookie",
			"snapfire_lil_shredder",
			"abaddon_death_coil",
			"abaddon_borrowed_time",
			"legion_commander_overwhelming_odds",
			"ember_spirit_flame_guard",
			"night_stalker_crippling_fear",
			"night_stalker_darkness",
			"custom_fireball"			
}