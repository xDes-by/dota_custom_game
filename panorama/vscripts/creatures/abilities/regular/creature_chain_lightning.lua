creature_chain_lightning = class({})

function creature_chain_lightning:OnSpellStart()
	if IsServer() then
...
function creature_chain_lightning:Zap(source, target, damage)
	table.insert(self.targets_hit, target)

	local bounce_pfx = ParticleManager:CreateParticle("particles/creature/chain_lightning.vpcf", PATTACH_ABSORIGIN_FOLLOW, source)
