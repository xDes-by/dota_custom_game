LinkLuaModifier("modifier_creature_flop", "creatures/abilities/regular/creature_flop", LUA_MODIFIER_MOTION_BOTH)

creature_flop = class({})

function creature_flop:ProcsMagicStick() return false end

function creature_flop:OnAbilityPhaseStart()
	if IsServer() then
...
function creature_flop:OnAbilityPhaseInterrupted()
	if IsServer() then
		self:GetCaster():RemoveGesture( ACT_DOTA_CAST_ABILITY_2 )
	end
end

function creature_flop:OnSpellStart()
	if IsServer() then
		...
		self:GetCaster():AddNewModifier( self:GetCaster(), self, "modifier_creature_flop", kv )
		EmitSoundOn("CreatureFlop.Cast", self:GetCaster())
	end
end

function creature_flop:TryToDamage()
	if IsServer() then
		...
		EmitSoundOnLocationWithCaster(self:GetCaster():GetOrigin(), "BossSmash.Land", self:GetCaster())
...
function modifier_creature_flop:IsStunDebuff()
	return true
end

--------------------------------------------------------------------------------

function modifier_creature_flop:IsHidden()
	return true
end

--------------------------------------------------------------------------------

function modifier_creature_flop:IsPurgable()
	return false
end

--------------------------------------------------------------------------------

function modifier_creature_flop:RemoveOnDeath()
	return false
end

--------------------------------------------------------------------------------

function modifier_creature_flop:OnCreated( kv )
	if IsServer() then
		if self.nHopCount == nil then
			...
			local nFXIndex = ParticleManager:CreateParticle("particles/creature/ogre_seal_warcry.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_creature_flop:OnDestroy()
	if IsServer() then
...
function modifier_creature_flop:DeclareFunctions()
	local funcs = 
	{
...
function modifier_creature_flop:CheckState()
	local state =
	{
...
function modifier_creature_flop:UpdateHorizontalMotion( me, dt )
	if IsServer() then
...
function modifier_creature_flop:UpdateVerticalMotion( me, dt )
	if IsServer() then
...
function modifier_creature_flop:OnHorizontalMotionInterrupted()
	if IsServer() then
...
function modifier_creature_flop:OnVerticalMotionInterrupted()
	if IsServer() then
		self:Destroy()
	end
end
...
function modifier_creature_flop:GetOverrideAnimation( params )
