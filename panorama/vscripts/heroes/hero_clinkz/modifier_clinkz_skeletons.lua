...
function modifier_clinkz_skeletons:OnCreated()
	if IsServer() then
		local arrows_ability = self:GetParent():FindAbilityByName("frostivus2018_clinkz_searing_arrows")
		arrows_ability:SetLevel(4)

		if (not arrows_ability:GetAutoCastState()) then arrows_ability:ToggleAutoCast() end
