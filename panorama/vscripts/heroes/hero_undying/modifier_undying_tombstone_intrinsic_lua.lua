...
function modifier_undying_tombstone_intrinsic_lua:GetModifierProcAttack_Feedback(params)
	...
	local modifier = params.target:AddNewModifier(self.parent, self.ability, "modifier_undying_tombstone_debuff_lua", {
		duration = self.debuff_duration * (1 - params.target:GetStatusResistance())
	})
