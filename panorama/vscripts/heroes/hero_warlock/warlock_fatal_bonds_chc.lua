...
LinkLuaModifier("modifier_fatal_bonds_chc_debuff", "heroes/hero_warlock/warlock_fatal_bonds_chc", LUA_MODIFIER_MOTION_NONE)

warlock_fatal_bonds_chc = class({})

function warlock_fatal_bonds_chc:OnSpellStart()
	...
	for _, affected_enemy_index in pairs(targets) do
		local enemy = EntIndexToHScript(affected_enemy_index)
		enemy:AddNewModifier(caster, self, "modifier_fatal_bonds_chc_debuff", {
			duration = enemy:ApplyStatusResistance(debuff_duration),
...
modifier_fatal_bonds_chc_debuff = class({})

function modifier_fatal_bonds_chc_debuff:IsDebuff() return true end
function modifier_fatal_bonds_chc_debuff:IsHidden() return false end
function modifier_fatal_bonds_chc_debuff:IsPurgable() return true end
function modifier_fatal_bonds_chc_debuff:GetAttributes() return MODIFIER_ATTRIBUTE_MULTIPLE end

function modifier_fatal_bonds_chc_debuff:ShouldUseOverheadOffset()
	return true
end

function modifier_fatal_bonds_chc_debuff:GetEffectName()
	return "particles/units/heroes/hero_warlock/warlock_fatal_bonds_icon.vpcf"
end

function modifier_fatal_bonds_chc_debuff:GetEffectAttachType()
	return PATTACH_OVERHEAD_FOLLOW
end

function modifier_fatal_bonds_chc_debuff:DeclareFunctions()
	local funcs = {
...
function modifier_fatal_bonds_chc_debuff:OnTooltip()
	return self:GetAbility():GetSpecialValueFor("damage_share_percentage")
end

if not IsServer() then return end

function modifier_fatal_bonds_chc_debuff:OnCreated(keys)
...
function modifier_fatal_bonds_chc_debuff:GetModifierIncomingDamage_Percentage(keys)
...
function modifier_fatal_bonds_chc_debuff:OnIntervalThink()
	local caster = self:GetCaster()
	...
	for _, target in pairs(self.share_targets) do
		if (not target:IsNull()) and target:IsAlive() and target:HasModifier("modifier_fatal_bonds_chc_debuff") and parent ~= target then
			...
			if self.current_damage > 0 then
				ApplyDamage({
					...
					damage_flags = DOTA_DAMAGE_FLAG_HPLOSS + DOTA_DAMAGE_FLAG_NO_SPELL_AMPLIFICATION + DOTA_DAMAGE_FLAG_NO_SPELL_LIFESTEAL + DOTA_DAMAGE_FLAG_BYPASSES_BLOCK
				})
			end
	...
	if not parent:IsAlive() or (target_count <= 0 and GameMode:IsTeamFightingCreeps(caster:GetTeam())) then
		self:Destroy()
	end
end

function modifier_fatal_bonds_chc_debuff:OnDestroy()
	self:OnIntervalThink()
end
