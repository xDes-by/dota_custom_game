...
function modifier_wisp_spirits_active:DestroySpirit(index, final)
	...
	for _, enemy in pairs(enemies) do
		...
		if self.parent:HasScepter() then
			enemy:AddNewModifier(self.parent, self.ability, "modifier_wisp_spirits_slow", { duration = 0.3 })
		end
	end
...
function modifier_wisp_spirits_active:OnIntervalThink()
	...
	if not self.control_ability or self.control_ability:IsNull() then self:Destroy() return end
