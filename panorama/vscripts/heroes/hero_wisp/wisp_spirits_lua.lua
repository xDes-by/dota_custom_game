...
function wisp_spirits_lua:Spawn()
	...
	self.scepter_gained_listener = EventDriver:Listen("Hero:scepter_received", function(self, event)
	...
	self.scepter_lost_listener = EventDriver:Listen("Hero:scepter_lost", function(self, event)
		if event.hero ~= self:GetCaster() then return end
		local modifier = event.hero:FindModifierByName("modifier_wisp_spirits_active")
...
function wisp_spirits_in_lua:ProcsMagicStick() return false end
