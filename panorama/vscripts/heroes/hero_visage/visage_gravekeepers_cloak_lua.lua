...
LinkLuaModifier("modifier_visage_stone_form_buff_lua", "heroes/hero_visage/modifier_visage_stone_form_buff", LUA_MODIFIER_MOTION_NONE )
...
function visage_gravekeepers_cloak_lua:OnSpellStart()
	...
	caster:AddNewModifier(caster, self, "modifier_rooted", { duration = delay })
	caster:StartGesture(self:GetCastAnimation())

	Timers:CreateTimer(delay, function()
		if not IsValidEntity(caster) or not caster:IsAlive() then return end

		caster:AddNewModifier(caster, self, "modifier_visage_stone_form_buff_lua", { duration = shard_duration })
	end)
end
