...
timbersaw_chakram_lua_return = class({})

LinkLuaModifier("modifier_timbersaw_chakram_thinker_lua", "heroes/hero_timbersaw/modifier_timbersaw_chakram_thinker_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_timbersaw_chakram_slow_lua", "heroes/hero_timbersaw/modifier_timbersaw_chakram_slow_lua", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_timbersaw_chakram_disarm_lua", "heroes/hero_timbersaw/modifier_timbersaw_chakram_disarm_lua", LUA_MODIFIER_MOTION_NONE)

timbersaw_chakram_lua.return_ability_name = "timbersaw_chakram_lua_return"
...
function timbersaw_chakram_lua:OnUpgrade()
	if not IsServer() then return end
	if self.is_scepter then return end

	local scepter_ability = self:GetCaster():FindAbilityByName("timbersaw_chakram_2_lua")
...
function timbersaw_chakram_lua:OnSpellStart()
	...
	local thinker = CreateModifierThinker(
		caster, 
		self, 
		"modifier_timbersaw_chakram_thinker_lua", -- modifier name
		{
	...
	local modifier = thinker:FindModifierByName("modifier_timbersaw_chakram_thinker_lua")
...
function timbersaw_chakram_lua_return:OnSpellStart()
...
timbersaw_chakram_2_lua.is_scepter = true
timbersaw_chakram_2_lua.return_ability_name = "timbersaw_chakram_lua_2_return"


function timbersaw_chakram_2_lua:Spawn()
	if not IsServer() then return end
	local caster = self:GetCaster()

	local main_ability = caster:FindAbilityByName("timbersaw_chakram_lua")
...
timbersaw_chakram_lua_2_return = class(timbersaw_chakram_lua_return)
