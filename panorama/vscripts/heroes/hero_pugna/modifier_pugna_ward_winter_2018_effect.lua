...
function modifier_pugna_ward_winter_2018_effect:OnCreated(kv)
    ...
    self.aura_mana_regen = ability:GetSpecialValueFor("aura_mana_regen")
    self.mana_multiplier = ability:GetSpecialValueFor("mana_multiplier")
end
...
function modifier_pugna_ward_winter_2018_effect:OnSpentMana(params)
    if IsServer() then
        ...
        local ward = self:GetCaster().hWard
        if not ward or ward:IsNull() then return end
        ...
        ParticleManager:SetParticleControlEnt(nFXIndex, 0, ward, PATTACH_POINT_FOLLOW, "attach_hitloc", ward:GetOrigin(), true)
        ParticleManager:SetParticleControlEnt(nFXIndex, 1, self:GetParent(), PATTACH_POINT_FOLLOW, "attach_hitloc", self:GetParent():GetOrigin(), true)
        self:AddParticle(nFXIndex, true, false, 0, false, false)
        EmitSoundOn("Hero_Pugna.NetherWard.Attack", ward)
        if not self.mana_multiplier then self.mana_multiplier = 1 end
