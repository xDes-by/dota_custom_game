...
function pugna_ward_winter_2018:OnSpellStart()
    if IsServer() then
		...
		if hWard ~= nil then
			local attacks_to_destroy_tooltip = self:GetSpecialValueFor("attacks_to_destroy_tooltip")
			hWard:SetBaseMaxHealth(attacks_to_destroy_tooltip)
			hWard:SetMaxHealth(attacks_to_destroy_tooltip)
			hWard:SetHealth(attacks_to_destroy_tooltip)
			hWard:SetBaseMagicalResistanceValue(100)
