...
LinkLuaModifier("modifier_item_aeon_disk_lua", "items/aeon_disk", LUA_MODIFIER_MOTION_NONE)
LinkLuaModifier("modifier_item_aeon_disk_lua_buff", "items/aeon_disk", LUA_MODIFIER_MOTION_NONE)
...
function modifier_item_aeon_disk_lua:CanProcPassive(damage)
	local health_threshold = self.parent:GetMaxHealth() * self.health_threshold_pct
	return IsValidEntity(self.ability) and self.ability:IsCooldownReady() and self.parent:GetHealth() - damage <= (health_threshold + 1)
end

function modifier_item_aeon_disk_lua:GetModifierAvoidDamage(keys)
	...
	if self:CanProcPassive(keys.damage) then
