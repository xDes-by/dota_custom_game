if CustomChatWheel == nil then CustomChatWheel = class({}) end

CustomChatWheel.heroes = {
	"abaddon",
	"alchemist",
	"ancient_apparition",
	"antimage",
...
CustomChatWheel.heroesvo = {
	...
	{
		"primal_beast_primal_happy_01",
		"primal_beast_primal_happy_02",
		"primal_beast_primal_deny_01",
		"primal_beast_primal_move_14",
		"primal_beast_primal_pain_07",
		"primal_beast_primal_shakeoff_01",
		"primal_beast_primal_underattack_08",
		"primal_beast_primal_attack_01",
	},
	{
		"puck_puck_laugh_01",
		"puck_puck_spawn_04",
...
function CustomChatWheel:Init()
	ListenToGameEvent("player_chat", Dynamic_Wrap(CustomChatWheel, "OnPlayerChat"), self)
end

_G.tPlayersChatWheelReady = {}
_G.tPlayersLanguages = {}
_G.tPlayersMuted = {}
...
SelectVO = function(keys)
	...
	local heroes = CustomChatWheel.heroes
	local selectedid = 1
	...
	if keys.num >= startheronums then
		...
		if not mesarrs[math.fmod(locnum,8)+1] or not heroes[math.floor(locnum/8)+1] then return end
		...
		if keys.num < (startheronums-8) then
			local mesarrs = CustomChatWheel.mesarrs
	...
	if selectedstr ~= nil and selectedid2 ~= nil then
		local heroesvo = CustomChatWheel.heroesvo
		...
		if votimer[keys.PlayerID] ~= nil then
			if GameRules:GetGameTime() - votimer[keys.PlayerID] > 5 + vousedcol[keys.PlayerID] and (phraseDoesntHasCooldown == nil or phraseDoesntHasCooldown == true) then
				...
				local chat = LoadKeyValues(GetChatWheelLocalizeFile(keys.PlayerID))
				local stringToChat = chat["dota_chatwheel_message_"..selectedstr]
				if not stringToChat then
					chat = LoadKeyValues("scripts/custom_chat_wheel/hero_chat_wheel_english.txt")
				end
				stringToChat = chat["dota_chatwheel_message_"..selectedstr]

				if heroesvo and heroesvo[selectedid] and heroesvo[selectedid2] then
					ChatSound(heroesvo[selectedid][selectedid2], keys.PlayerID)
					CustomChat:MessageToAll(stringToChat, keys.PlayerID)
...
function GetChatWheelLocalizeFile(playerId)
	if _G.tPlayersLanguages[playerId] == 'russian' then
		return "scripts/custom_chat_wheel/hero_chat_wheel_russian.txt"
	elseif _G.tPlayersLanguages[playerId] == 'schinese' then
		return "scripts/custom_chat_wheel/hero_chat_wheel_schinese.txt"
	elseif _G.tPlayersLanguages[playerId] == 'tchinese' then
		return "scripts/custom_chat_wheel/hero_chat_wheel_tchinese.txt"
	else
		return "scripts/custom_chat_wheel/hero_chat_wheel_english.txt"
	end
end
...
function ChatSound(phrase, source_player_id)
	...
	for _, hero in pairs(all_heroes) do
		if hero:IsRealHero() and hero:IsControllableByAnyPlayer() then
			...
			if not _G.tPlayersMuted[player_id] then _G.tPlayersMuted[player_id] = {} end
...
RegisterCustomEventListener("checkWheel", function(data)
	local PlayerID = data.PlayerID
	if not PlayerID then return end
	local player = PlayerResource:GetPlayer(PlayerID)
	if not player or player:IsNull() then return end
	if _G.tPlayersChatWheelReady[PlayerID] then
...
RegisterCustomEventListener("setlanguage", function(data)
	...
	local player = PlayerResource:GetPlayer(PlayerID)
	if not player or player:IsNull() then return end
	_G.tPlayersLanguages[PlayerID] = data.language
end)


CustomChatWheel:Init()
